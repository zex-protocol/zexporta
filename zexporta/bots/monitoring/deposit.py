...
async def get_user_balance(client: httpx.AsyncClient, user_id: UserId, symbol: str) -> Decimal:
    balance = [
        user_asset.free for user_asset in (await get_user_asset(client, user_id=user_id)) if user_asset.asset == symbol
    ]
    if len(balance) == 0:
        return Decimal(0)
